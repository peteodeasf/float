...
| **An accommodation needs a state the clinician can see and change** | Not built. `status` on `AccommodationBehavior` is set to "active" when it is created and then never changed by anything — nothing reads or writes it. So when the weekly focus moves on, the previous accommodation vanishes from the app entirely: the parent only logs against the current focus, and nothing asks about the old one again. The book: *"Make sure you do not go back... you will set the stage for relapse."* Give it a state — not started, current focus, stopped — show it to the clinician on the parent plan panel, and let them move one back to the focus. **The app does not try to detect drift.** The clinician finds that out by asking in session, which is Peter's call and consistent with the rest of Float. | M |
