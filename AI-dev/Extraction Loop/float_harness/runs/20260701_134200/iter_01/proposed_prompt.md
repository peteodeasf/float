...
> Draft v3 (merged: v2 clinical definitions + tuned classification guardrails). Self-contained:
...
- **Distress is not itself a behavior type.** Signs of distress alone — crying,
  getting upset, feeling sick, almost vomiting, shaking, complaining of stomachaches —
  are NOT `safety`, `avoidance`, or `escape`. Do not create a separate behavior for a
  distress reaction, and do not classify a distress reaction as `safety`. Instead,
  attach the distress detail to the description of the behavior it accompanies
  (e.g. an avoidance behavior's description may note that the child got so upset he
  almost vomited), or omit it if it stands alone with no associated behavior. When a
  child refuses or leaves *while* distressed, that is a single avoidance or escape
  behavior, not distress plus another behavior.
- **Do not label something `safety` (e.g. "seeking reassurance") unless the note shows
  ...
  they are scared, upset, or explaining why, is not by itself reassurance-seeking or a
  safety behavior — do not add that framing unless the note supports it.
...
- **Do not add editorial or meta commentary as a behavior.** Never create a behavior
  entry that comments on the note itself or on your own uncertainty (e.g. "Hard to
  tell from the note whether these behaviors continued..."). If a note is genuinely
  ambiguous, use a single `unclear` behavior; otherwise record only behaviors the
  parent actually reported.
...
- **Preserve the parent's wording, including pronouns and speaker.** When you quote or
  paraphrase something a parent or child said, keep the original speaker and pronouns
  intact. If the parent wrote that the child said "I'm the worst mother ever" about the
  parent, do not shift the pronoun (do not turn it into "she's the worst mother ever").
  Attribute quotes to whoever the note attributes them to.
- **Capture every accommodation the note describes**, including brief or passive ones
  such as a parent encouraging the child to try again next time, offering comfort, or
  reassuring. If an adult changed their own behavior in response to the child's
  anxiety, list it as an accommodation even if it is stated in a single phrase.
...
- **Accommodation includes passive adult responses** — giving space, allowing delays,
  waiting, encouraging the child to try again — not only active facilitation. Anger or
  pressure is not accommodation.
- **Split compound or contradictory notes** into separate behaviors and classify each.
  ...
  avoidance, then escape. Do not split a single action into multiple behaviors, and do
  not split a distress reaction off from the behavior it accompanies.
...
**Note (distress accompanies a refusal — one behavior, not two):**
"(Bedtime, fear 7/10) M refuses to sleep in his own room and gets so upset he almost
vomits. I let him sleep in our bed and encouraged him to try his own room next time."

**Output:**
```
{"situations":[{"name":"Bedtime","fear_rating":7,"behaviors":[
{"order":1,"type":"avoidance","description":"Refuses to sleep in his own room; gets so upset he almost vomits"}],
"accommodations":[
{"description":"Parent let him sleep in their bed"},
{"description":"Parent encouraged him to try his own room next time"}]}]}
```
...
- Offer encouragement or comfort in response to distress, including encouraging the child to try again next time