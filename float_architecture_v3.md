...
**HasslesList**
Belongs to the patient and treatment plan. Records how the patient's anxiety interferes with desired activities — birthday parties, class participation, social events. Used as a motivation anchor at the start of treatment and as an outcome measure over time. To be completed by the patient or parent depending on age.
...
| 10 | Hassles list workflow — at what point in the process, who completes it, how it connects to the trigger situations list | Dr. Walker | New tool identified in v0.3 brief |
| 11 | Multi-org support timing — Phase 1 single org or multi-org from start | Product decision | Single org is simpler but migration later has cost |
| 12 | Clinical content library — scope, ownership, and data model | Dr. Walker + Product | See design decision below — pre-launch requirement |

---

## 12. Clinical Content Library (Design Decision)

**Status: Not yet built — pre-launch requirement**

### The problem

Every patient's treatment plan currently requires practitioners to type trigger situations and avoidance behaviors from scratch. For a school social worker seeing 20 anxious teens, this is slow. More importantly, without a shared vocabulary across patients, population-level analysis is impossible — you can't aggregate outcomes across patients if "eating in cafeteria" is spelled differently in every record.

### What the library is

A curated set of canonical trigger situations and avoidance/safety behaviors that practitioners can browse and add to a patient's plan with one click. Each item in the library has:

- A canonical name and description (defined by Dr. Walker)
- A suggested distress thermometer range
- A behavior type (avoidance / safety / ritual)
- Tags for anxiety presentation type (social anxiety, perfectionism, separation anxiety, etc.)

When a practitioner adds a library item to a patient's plan, it creates an instance in the patient record — linked back to the library item by ID, but editable for patient-specific nuance.

### Why this matters for analytics

If trigger situations and behaviors are linked to library items, Float can aggregate across patients:
- Which situations are most prevalent in the population?
- Which behaviors are hardest to relinquish (highest DT when refraining)?
- Which ladder configurations produce the best BIP outcomes?
- Which situations respond fastest to exposure work?

This is the foundation of Float's outcomes research story and eventual value to school systems and health systems.

### Data model additions needed

```sql
-- Platform-level canonical library
CREATE TABLE clinical_library_items (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  item_type TEXT NOT NULL CHECK (item_type IN ('trigger_situation', 'avoidance_behavior')),
  name TEXT NOT NULL,
  description TEXT,
  behavior_type TEXT,  -- for behaviors: avoidance | safety | ritual
  suggested_dt_min DECIMAL(3,1),
  suggested_dt_max DECIMAL(3,1),
  anxiety_tags TEXT[],  -- social_anxiety | perfectionism | separation | etc
...
Add `library_item_id` as an optional foreign key on `trigger_situations` and `avoidance_behaviors` tables — null for free-text entries, populated when added from the library.

### UI implications

On the trigger situation and behavior add forms, add a **"Browse library"** option that opens a searchable panel of canonical items filtered by anxiety presentation type. One-click add, then edit for patient-specific nuance.

### Ownership question (open)

Does Dr. Walker own the library content entirely, or can org-level practitioners contribute items that get reviewed and promoted to the platform library? The data model supports both — `created_by` field handles attribution and `is_active` handles curation.

### When to build

Before general availability — not needed for Dr. Walker's initial pilot review (she can use free-text), but required before onboarding real school practitioners at scale.

---

*Float Technical Architecture — Living Document — v0.2 — April 2026*
