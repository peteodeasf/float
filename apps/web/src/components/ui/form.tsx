...
export const INPUT: React.CSSProperties = { width: '100%', padding: '8px 10px', boxSizing: 'border-box', fontSize: '14px', border: '1px solid var(--float-border-strong)', borderRadius: 'var(--float-radius-control)' }
export const ERROR_BOX: React.CSSProperties = { background: '#fef2f2', border: '1px solid #fecaca', borderRadius: 'var(--float-radius-control)', padding: '10px 12px', marginTop: '14px', fontSize: '13px', color: 'var(--float-danger)' }

export function Field({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <label style={{ display: 'block' }}>
      <span style={{ display: 'block', fontSize: '12px', fontWeight: 600, color: 'var(--float-text-secondary)', marginBottom: '4px' }}>{label}</span>
      {children}
      {hint && <span style={{ display: 'block', fontSize: '11px', color: 'var(--float-text-hint)', marginTop: '4px' }}>{hint}</span>}
    </label>
  )
}
