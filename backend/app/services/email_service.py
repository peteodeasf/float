...
<body style="margin:0; padding:0; background:#fafafa; font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif;">
  ...
  <div style="max-width:480px; margin:0 auto; padding:32px 24px;">
    <div style="background:#ffffff; border-radius:10px; padding:32px 28px; border:1px solid #e2e8f0;">
      ...
      <p style="font-size:18px; font-weight:600; color:#0f172a; margin:0 0 12px;">
        You've been invited to Float
      </p>

      <p style="font-size:15px; color:#475569; line-height:1.6; margin:0 0 20px;">
        Your clinician has invited you to Float{about}. Log in to see the plan you'll
        be working on together and track how it's going, one step at a time.
      </p>
...
Your clinician has invited you to Float{about}. Log in to see the plan.
...
"""
    ...
    try:
        ...
        logger.info(f"Parent invitation email sent to {to_email}")
        return True
    except Exception as e:
...
"""
    ...
    try:
        resend.Emails.send({
            "from": f"{settings.RESEND_FROM_NAME} <{settings.RESEND_FROM_EMAIL}>",
            "to": [to_email],
            "subject": "You've been invited to Float",
            "html": html_body,
            "text": text_body,
        })
